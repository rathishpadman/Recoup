...
| OpenAI evidence vector-store recovery | Independent implementation and senior-diff review | The configured store returned provider HTTP 404 under the same key used successfully for model calls. A replacement four-dossier store passed exact Maya retrieval checks, and an eighth Source Readiness tile was added. Two senior reviews found and closed provider-ID exposure, reprovisioning idempotency, bounded-request/body timeout, aggregate/per-file manifest, wrong-type, and mixed-record-scope gaps. | Local remediation verified; final review and production approval gate pending; production not changed | `docs/superpowers/plans/2026-07-12-maya-vector-store-recovery.md`, `config/openAiEvidenceVectorManifest.ts`, `src/services/openAiEvidenceVectorReadiness.ts`, `scripts/validateOpenAiEvidenceVectorStore.ts`, `tests/unit/openai-evidence-vector-readiness.test.ts`, `tests/unit/openai-evidence-vector-store-provisioner.test.ts`, `tests/unit/openai-evidence-vector-store-validator.test.ts` |
...
- `package.json` exposes the side-effecting lifecycle as `npm run provision:openai-evidence-vector-store`; it is not part of `npm run verify`. Re-running it against the exact completed governed manifest performs zero uploads, while ambiguous nonempty stores fail closed.
...
### OpenAI Evidence Vector Store Recovery

- The previously configured vector store returned OpenAI HTTP 404 while the same configured key successfully executed the pinned model call, isolating the root cause to a stale/deleted store rather than key mismatch.
- A replacement store was created locally with four governed synthetic dossiers for `S1-L1`, `S3-L1`, `S6-L1`, and `S8-L1`; its provider batch completed before validation.
- Direct semantic searches across all twenty Maya deduction lines returned exactly the four expected document types and zero evidence for every nonmatching line.
- The Connector Readiness model now contains eight ordered sources, adding `OpenAI Vector Store` after `Contract Repo` and before `MCP Gateway`; missing or failed provider health remains visibly blocked.
- The complete post-remediation local guard passed with governed environment values: 172 test files / 1,569 tests, 157-module dependency boundaries, and release readiness.
- Premium browser E2E passed for Maya text/voice, David, CFO, FinOps, approvals, and responsive routes. Real-backend Maya E2E passed all eight work items / twenty lines plus five live OpenAI agent queries and fifty backend trace rows.
- Two independent senior reviews identified provider-ID exposure, duplicate uploads on reuse, unbounded request/body handling, incomplete aggregate and per-file manifest readiness, wrong-type evidence acceptance, and incomplete/mixed-record metadata acceptance. The implementation and tests close those findings; the final read-only review found no code-level release blocker.
- Render and Vercel production settings remain unchanged. Updating the provider ID, merging, and production deployment require a separate explicit owner approval after local remediation is complete.

Evidence: `docs/superpowers/plans/2026-07-12-maya-vector-store-recovery.md`, `scripts/provisionOpenAiEvidenceVectorStore.ts`, `scripts/validateOpenAiEvidenceVectorStore.ts`, `src/services/openAiEvidenceVectorReadiness.ts`, `src/services/sourceHealthPoller.ts`, `src/services/cockpitModel.ts`, `tests/unit/openai-evidence-vector-readiness.test.ts`, `tests/unit/openai-evidence-vector-store-provisioner.test.ts`, `tests/unit/source-health-poller.test.ts`, `tests/unit/cockpit.test.ts`, `tests/e2e/cockpit-premium-e2e.ts`, `tests/e2e/maya-real-backend-e2e.ts`.
