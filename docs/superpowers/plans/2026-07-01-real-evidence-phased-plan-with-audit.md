...
Current result: PARTIAL / RELEASE GATE STILL BLOCKED. `npm.cmd run test:e2e:maya-real` passed locally against `http://127.0.0.1:4318`: shell/readiness/force-refresh passed, canonical browse loaded 20 line items / 8 work items, and 5 live Maya query work items completed with 50 backend trace rows. SAP source health stayed degraded because the SAP service was temporarily down, but the live OpenAI Agents SDK + MCP `query.answer` path completed with selected canonical evidence proof instead of `blocked_live_agent_trace`. Focused gates also passed: `npm.cmd run test -- tests/unit/forensics-query-session.test.ts tests/unit/live-forensics-stream.test.ts tests/unit/conductor-hooks.test.ts tests/unit/retrieval-tools.test.ts` passed 4 files / 62 tests; `npm.cmd run test -- tests/unit/cockpit-api.test.ts` passed 1 file / 121 tests; `npm.cmd run test -- tests/invariants/maya-shadcn-qa-contract.test.ts` passed 1 file / 35 tests; latest full `npm.cmd run verify` passed with 124 files / 1038 tests, and a later full `npm.cmd run verify` at the final preview code point passed with 124 files / 1050 tests. Final protected Preview proof now exists, but `npm.cmd run verify:real-evidence-hardening`, `npm.cmd run verify:real-evidence-visual`, and `npm.cmd run check:real-evidence-proof` remain release-blocked until approved production/public-alias post-capture, pixel-diff proof, canary proof, and promotion-time provider/env proof exist. Cross-browser proof is intentionally skipped locally by user instruction; Chromium a11y passed.

- [x] **Step 2: Run preview smoke before touching the public alias**
...
Current preview result on 2026-07-02: PASS for protected Preview smoke, not production proof. The final protected Preview tested was `https://recoup-hetylxq9o-hackathonopenai.vercel.app`, served from the preview code deployment for `430565d`, and no production alias promotion was run. `npm.cmd run test:e2e:shared-surfaces` against the final Preview passed, and `npm.cmd run capture:real-evidence-audit` wrote `docs/audit/real-evidence-preview/2026-07-02-430565d/manifest.json` plus 19 screenshots. The manifest records 19 captures, 19 HTTP 200 statuses, 19 screenshots, zero capture errors, and zero console-error routes. The four required Maya evidence states (`selected case`, `evidence provenance drawer`, `query answer panel`, and `approval audit panel`) each show `EVD-POD-S3-L1`, `EVD-REMIT-S3-L1`, `RECON-S3-L1`, visible hashes/provenance, and a visible same-origin POD PDF link at `/api/forensics/evidence-documents/EVD-POD-S3-L1` with HTTP 200, `application/pdf`, and positive byte length. Superseded preview attempt `docs/audit/real-evidence-preview/2026-07-01-preview-2/manifest.json` remains a historical protected-login failure and is no longer the current preview proof.
...
This section answers the final acceptance question about whether each phase ran its targeted gates and `npm.cmd run verify` before the next phase began. The current Chronology verdict is **PARTIAL**: early local phase gates and the final protected Preview smoke are documented, but canary, public-alias production capture, and production post-proof remain open. Local/Preview green tests are not production cutover proof.
...
| Phase 6 -> 6.5 | PARTIAL | Local unit/full verify and later local e2e movement exist. | Preview smoke now exists, but no public-alias proof. | Phase 6 exit was recorded as partial/release-blocked. |
| Phase 6.5 -> 7 | PARTIAL | Local hardening gates passed for Chromium/local proof and shared surfaces. | Protected Preview route/media proof exists; cross-browser, production pixel-diff, approved public-alias post-capture, and final proof-readiness gates are still missing. | Phase 6.5 release gate remains blocked. |
| Phase 7 closeout | NOT PROVEN | Local `npm.cmd run verify`, read-only Supabase source-table proof, and protected Preview route/media proof exist. | Canary, production screenshots, production media proof, and public-alias smoke are missing. | Do not mark production-ready. |
...
- [ ] A POD PDF/image/media artifact or safe document link is visible in the production frontend and load-verified. Protected Preview proof is green for `EVD-POD-S3-L1`; production/public-alias proof remains pending.
...
- [x] Preview deployment smoke passes before the public production alias is promoted or declared fixed: final protected Preview capture `docs/audit/real-evidence-preview/2026-07-02-430565d/manifest.json` has 19 captures, 19 HTTP 200 statuses, 19 screenshots, zero capture errors, zero console-error routes, visible `EVD-POD-S3-L1`, visible `RECON-S3-L1`, and HTTP-verified POD PDF proof on the four Maya evidence states.
- [ ] Canary proof passes for approved line IDs before production mode changes to `authoritative`.
- [x] Maya evidence UI exposes `EVD-*` and `RECON-*` when the backend/read model supplies canonical evidence and receipts; local and protected Preview visibility for `EVD-POD-S3-L1` and `RECON-S3-L1` are proven, while production/public-alias visibility remains unchecked above.
...
- [x] `docs/independent-audit-log.md` contains before/current/preview/production audit verdict movement for the pasted audit dimensions, with final protected Preview proof marked pass/not-production and production/post verdicts explicitly blocked until user testing, canary/public-alias capture, production POD/media proof, and promotion-time production preflight pass.
