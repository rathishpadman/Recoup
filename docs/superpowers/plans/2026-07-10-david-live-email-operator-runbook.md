...
- Current local ignored env is David-send-ready for non-webhook pieces: owner supplied the David sender and Harbor AP recipient, `RECOUP_CREDIT_NEGOTIATION_ROOT_CATCHALL_APPROVED=approved` is encoded locally, conservative test guardrails are configured (`10` inbound events per `60000` ms; retry batch limit `3`), and `RECOUP_EMAIL_SEND_PRINCIPALS` includes the David demo principal without removing Maya. Do not commit local env values.
- Current local contact seed is complete: `npm run seed:credit-negotiation-contacts` returned `status=seeded`, `rows=1`, `readBack=true`, and hash-only proof for `ACC-HAR` role `ap`.
- Current live-email blockers are now only provider/public-route items: `RESEND_INBOUND_SIGNING_SECRET`, Resend `email.received` webhook registration to the Recoup David inbound route, and the public Recoup inbound route returning `404` until an owner-approved deploy/exposure step happens.
...
- Owner approved `north-bay.dev` root catch-all safety in chat on 2026-07-10; local env now encodes the approval, but production/provider movement still requires explicit owner approval.
- Corrected production boundary: no code deploy has happened, and no Resend webhook has been registered or modified. David-specific Vercel Production environment variable names were staged before the owner clarified the boundary; owner must choose whether to keep those staged env keys pending deploy approval or remove them before any further production/provider work.
