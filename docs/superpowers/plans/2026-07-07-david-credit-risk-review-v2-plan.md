...
**Trigger:** Owner requested a full public-alias browser check after prod movement, including landing tabs/buttons, Maya 8 scenarios, Maya live/voice query, David 4 scenarios, and David query. **Owner reminder:** deploy success is not release success; after prod movement, the full browser matrix below must pass. **Hard gate:** after merge/deploy, do not call the release complete until every item below passes against the stable production alias and the deployment ID/commit are recorded.
- [ ] **Step 0: Run the repeatable post-prod smoke harness.** From the verified `main` worktree after Render and Vercel deploys are live, run `RECOUP_PROD_APP_URL=https://recoup-self-eta.vercel.app`, `RECOUP_PROD_API_URL=https://recoup-api.onrender.com`, then `npm run smoke:post-prod`. Expected: `output/playwright/post-prod-public-smoke/post-prod-public-smoke-results.json` passes landing tabs/buttons, Maya live text query/cache, Maya realtime voice bridge, David live account queries, and David backend-vs-Supabase credit-source proof. Do not use `RECOUP_POST_PROD_SKIP_VOICE=1` for release completion unless the owner explicitly accepts a manual voice-query replacement.
...
- [ ] **Step 2: Landing buttons and CTAs.** Click every business-visible landing button: header Maya, header David, hero Maya, hero David, demo Maya, demo David, bottom CTA(s), and any secondary visible CTA/button on the landing page. Expected: Maya CTAs route to `/login?loginId=Maya`, David CTAs route to `/login?loginId=david`, every other button either navigates to its documented target or has owner-approved inert behavior, no CTA is disabled/dead, and no button regresses from the pre-release behavior.
