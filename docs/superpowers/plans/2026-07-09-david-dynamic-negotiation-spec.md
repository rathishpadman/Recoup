...
- Done locally with focused tests: policy exact-row parser/fail-closed guard (P0.1), Harbor simulation endpoint foundation (P0.2), deterministic EV/optimizer core (P1.2 core compute, without `credit_deal_scenarios` persistence yet), Supabase simulated-source tables seeded for Harbor (P1.1 source tables; artifact: `docs/superpowers/plans/2026-07-09-david-credit-negotiation-supabase-migration.sql`), deal workbench endpoint/UI + browser e2e (P1.3), and governed `credit_negotiation.draft_structures` MCP/live-query backend wiring (P2.1).
- Latest verification passed: `npm run test:e2e:david-v2` (David 4-account live LLM queries with `live_openai_agents`, Harbor optimizer workbench, approval receipt/read-back, Maya containment regression), `npm run test:e2e:shared-surfaces`, `npm run test -- tests/unit/credit-v2-api.test.ts tests/unit/credit-negotiation-drafts.test.ts tests/unit/live-forensics-stream.test.ts tests/unit/mcp-gateway.test.ts tests/unit/deal-optimizer.test.ts tests/unit/deal-expected-value.test.ts` (6 files / 50 tests), `npm run lint`, `npm run typecheck`, and `git diff --check`.
- Partially done: workbench uses backend-priced labels and synthetic badges; optimizer returns source/policy/run hashes but does not yet persist `credit_deal_scenarios`; copilot can surface a priced `negotiationDraft` from a live tool receipt, but the dedicated copilot UI affordance/e2e remains pending.
- Explicitly not done yet: P3 email negotiation. No implemented David email route, inbound Resend webhook, negotiation round state machine, or email-specific unit/e2e tests exist yet. Do not claim "email negotiation complete" until P3.1-P3.4 pass.
