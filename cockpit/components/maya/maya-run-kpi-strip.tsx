...
export function MayaRunKpiStrip({ actionInbox, items, recoveryTracker }: MayaRunKpiStripProps) {
  ...
  return (
    <section className="grid min-w-0 gap-2 md:grid-cols-3 xl:grid-cols-6" aria-label="Forensics run KPIs">
      {cards.map((card, index) => {
        ...
        return (
          <Card className="min-h-[168px] justify-between rounded-lg shadow-none" key={key} size="sm">
            <CardHeader className="gap-4">
              <div className="flex min-w-0 items-center justify-between gap-3">
                <div className={`flex size-11 shrink-0 items-center justify-center rounded-lg border ${kpiIconTone(index)}`}>
                  {renderKpiIcon(index)}
                </div>
                <Tooltip>
              ...
              <CardDescription className="min-h-8 text-xs leading-4">{label}</CardDescription>
            </CardHeader>
            <CardContent className="grid min-w-0 gap-4">
              <CardTitle className="min-w-0 overflow-hidden text-3xl font-semibold leading-none tabular-nums text-ellipsis">
                {value}
              </CardTitle>
