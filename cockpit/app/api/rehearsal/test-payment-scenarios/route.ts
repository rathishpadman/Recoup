...
export async function GET(request: Request): Promise<Response> {
  ...
  const authHeaders = buildVerifiedHumanAuthHeaders(runtimeEnv, request.headers, {
    ...
    proxyRequest: { body, method: request.method, path: "/rehearsal/test-payment-scenarios" }
  });
  ...
  try {
    const upstream = await fetch(
      `${runtimeEnv.RECOUP_API_URL ?? "http://127.0.0.1:4317"}/rehearsal/test-payment-scenarios`,
      {
