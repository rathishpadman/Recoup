...
export async function POST(request: Request): Promise<Response> {
  ...
  const authHeaders = buildVerifiedHumanAuthHeaders(runtimeEnv, request.headers, {
    ...
    proxyRequest: { body, method: "POST", path: "/credit/query" }
  });
  ...
  try {
    const upstream = await fetch(`${runtimeEnv.RECOUP_API_URL ?? "http://127.0.0.1:4317"}/credit/query`, {
      body,
      cache: "no-store",
      headers: {
    ...
    return Response.json({ error: "Credit risk query service unavailable." }, { headers: noStoreHeaders(), status: 502 });
  }
}
