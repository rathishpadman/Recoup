...
| SAP OData read-only connector | Resolved read-only Basic/OAuth baseline; `Tools_data` invoice mapping targets `ZUI_BILLINGDOCUMENTFS_0001/C_BillingDocumentFs` with `sap-client` routing | `src/adapters/sapOData.ts`, `skills/sap-odata-access/SKILL.md`, `tests/unit/sap-odata.test.ts` |
| Enterprise connector strategy | Foundation implemented from owner-ratified Day-1 tunables and source strategy: SAP remains live read-only; non-SAP synthetic Supabase static tables become `ready_synthetic` only after a schema probe verifies `Tools_data` tables, with live contracts deferred to VERIFY-V3 | `src/adapters/enterpriseReadOnly.ts`, `src/adapters/connectorRegistry.ts`, `src/memory/supabaseStore.ts`, `docs/supabase-memory-schema.sql`, `tests/unit/enterprise-connectors.test.ts`, `tests/invariants/connector-readiness.test.ts` |
...
- SAP sandbox auth/query mapping now uses the `docs/Tools_data` read-only invoice service `ZUI_BILLINGDOCUMENTFS_0001/C_BillingDocumentFs` with `SAP_ODATA_CLIENT` / `sap-client`; synthetic invoice IDs without a numeric SAP suffix fail closed instead of becoming live reads. Scope and tenant may remain blank when the SAP endpoint does not require OAuth; broader O2C SAP depth remains metadata/readiness proof until deterministic record IDs exist for each entity.
- Bureau, remittance/EDI, document repository, and TPM source depth is no longer blocked on owner answers: `CODEX_BUILD_ANSWERS.md` supplies the strategy to use synthetic Supabase static tables behind the canonical source ports for Day 1. The readiness model now requires a Supabase schema probe for the `docs/Tools_data` tables before reporting non-SAP sources as `ready_synthetic`; full table-reading adapters and live source contracts remain deferred to VERIFY-V3.
- Expert-owned constants are supplied in `CODEX_BUILD_ANSWERS.md`; the repo now carries a config-as-code bootstrap seed and `recoup_config` seed rows for Day-1 tunables, while the DB-backed runtime loader and VERIFY-PROD calibration remain open before final operating policy.
...
- Implement the DB-backed governed config loader for the supplied arbitration P&L weights, R-score weights, R-drift trigger, gaming gates, partial-hold config, accuracy bars, and seed; config-as-code bootstrap rows exist in repo, and VERIFY-PROD calibration remains required before production use.
- Resolve the remaining `[VERIFY]` flags from `CODEX_BUILD_ANSWERS.md`: embeddings model id, Codex build-model id, and SAP sandbox instance.
...
- SAP remains read-only: the demo invoice mapping targets `ZUI_BILLINGDOCUMENTFS_0001/C_BillingDocumentFs`; billing-item, sales-order, credit, dispute, and accrual services remain metadata/readiness proof until deterministic record IDs and key predicates are added.
- Implement full synthetic table-reading adapters for bureau, docs, remittance/EDI, and TPM when source retrieval expands beyond readiness; keep readiness blocked when the Supabase probe returns 404/not-exposed or finds unsafe shadow statuses such as `SENT_TO_SAP`, `SUBMITTED_TO_PORTAL`, or `SAP_STAGE_WRITE`; real non-SAP live adapters remain deferred to VERIFY-V3 live non-SAP contracts.
