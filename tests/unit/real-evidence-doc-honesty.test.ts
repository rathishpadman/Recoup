...
describe("real evidence release documentation honesty", () => {
  ...
  it("keeps the phase plan honest about hardening and live provider gates", () => {
    ...
    expect(plan).toContain("Phase 6.5 exit gate remains blocked");
    ...
    expect(plan).toContain("Current result: BLOCKED FAIL-CLOSED for the full hardening/release gate");
    ...
    expect(plan).toContain("Preview deployment attempt result");
    expect(plan).toContain("Vercel deployment protection redirects the browser to `https://vercel.com/login`");
    expect(plan).toContain("docs/audit/real-evidence-preview/2026-07-01-preview-2/manifest.json");
    expect(plan).toContain("proofScope:\"preview\"");
    expect(plan).toContain("releaseProof:false");
  ...
  it("keeps the independent audit log explicit about baseline, local movement, and production blockers", () => {
    ...
    expect(auditLog).toContain("PREVIEW DEPLOYED BUT BLOCKED BY VERCEL DEPLOYMENT PROTECTION");
    ...
    expect(auditLog).toContain("Preview deployment attempt on 2026-07-02");
    expect(auditLog).toContain("proofScope:\"preview\"");
    expect(auditLog).toContain("releaseProof:false");
    expect(auditLog).toContain("Vercel deployment protection");
  ...
  it("keeps the restart handoff aligned with current cutover blockers", () => {
    ...
    expect(handoff).toContain("Preview deployment checkpoint on 2026-07-02");
    expect(handoff).toContain("dpl_E3fhiU1v2fhX5H1ND2kD8geRHybW");
    expect(handoff).toContain("Vercel deployment protection");
    expect(handoff).toContain("proofScope: \"preview\"");
    expect(handoff).toContain("releaseProof: false");
    expect(handoff).toContain("approved post-implementation manifest is absent");
