...
describe("S5 cockpit business-logic boundary", () => {
  ...
  it("keeps the approved SaaS landing mockup content in the public page", () => {
    ...
    expect(root).not.toContain("McKinsey");
    expect(root).not.toContain("RVCF");
    expect(root).not.toContain("UpClear");
    expect(root).not.toContain("VIDEO_ID_HERE");
    expect(root).toContain("gpt-5.4 family, gpt-realtime-2, gpt-4o-mini-transcribe");
    expect(root).not.toContain("GPT-5.5");
    expect(root).not.toContain("GPT-4.1");
    expect(root).not.toContain("ArchitectureInfographic");
    expect(root).toContain("Recoup agentic order-to-cash visual: evidence flows through governed approval");
    expect(root).toContain("/recoup-agentic-hero-visual.png");
    expect(root).not.toContain("POD-7782");
    expect(root).not.toContain("Shortage claim DED-4411 is invalid");
    expect(root).toContain("Recoup architecture: read-only evidence in, human-approved action out");
    expect(root).toContain("/recoup-tech-architecture-infographic.png");
    expect(root).not.toContain("/architecture-diagram.png");
    expect(root).not.toContain("overflow-x-auto rounded-[10px] border border-border/70 bg-muted p-1");
    expect(root).not.toContain("data-active:shadow-sm");
    expect(root).toContain("grid h-auto w-full grid-cols-6");
    expect(root).toContain("group-data-horizontal/tabs:h-auto");
    expect(root).toContain("h-8 min-h-0");
    expect(root).toContain("items-stretch gap-5");
    expect(root).toContain('data-testid="recoup-landing-invariants-card"');
    expect(root).toContain('data-testid="recoup-landing-run-flow-card"');
    expect(root).toContain("30 invariant controls tracked");
    expect(root).toContain("David - Weekly Credit Risk Review");
    expect(root).toContain("Review the 4-account weekly risk queue");
    expect(root).toContain("Negotiate live email terms through a human-approved send gate");
    expect(root).toContain("David negotiation workbench");
    expect(root).toContain("Maya Patel");
  ...
  it("wires Maya shadcn query and approval through credential-gated browser/API boundaries", () => {
    ...
    const emailDraftDialog = readFileSync("cockpit/components/maya/email-draft-dialog.tsx", "utf8");
    ...
    expect(caseWorkspace).toContain('data-testid="maya-evidence-fact-card"');
    expect(caseWorkspace).toContain('data-testid="maya-evidence-fact-row"');
    expect(caseWorkspace).toContain('data-testid="maya-evidence-provenance-disclosure"');
    expect(caseWorkspace).toContain('testId="maya-evidence-provenance-row"');
    expect(caseWorkspace).toContain("Document");
    expect(caseWorkspace).toContain("Source");
    expect(caseWorkspace).toContain("Status");
    expect(caseWorkspace).not.toContain("Additional proof fields pending");
    expect(caseWorkspace).not.toContain("Backend evidence packet");
    expect(caseWorkspace).not.toMatch(
    ...
    expect(caseWorkspace).not.toContain("fetch(");
    expect(caseWorkspace).not.toContain("/api/");
  ...
  it("keeps Maya root sidebar sections and valid verdict surfacing read-model backed", () => {
    ...
    expect(surface).toContain("buildSourcePillState(connectors.sourceTiles)");
    expect(shell).not.toMatch(/\b(?:Cases|Evidence)\b/u);
    expect(surface).not.toContain('data-testid="maya-valid-deduction-signal"');
    expect(table).toContain("verdictBadgeVariant(item.verdict)");
    expect(table).toContain("data-verdict={item.verdict}");
    expect(workspace).toContain("verdictBadgeVariant(selectedWorklistItem.verdict)");
